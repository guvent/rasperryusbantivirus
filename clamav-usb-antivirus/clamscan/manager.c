...
#include "lcdmenu.c" // lcd ...............
#include "runtime.c" //  runtime.........
#include "nextion.c"
...
static void _scandirs(const char *dirname, struct cl_engine *engine, const struct optstruct *opts, unsigned int options, unsigned int depth, dev_t dev);

static int jump_to_mainmenu();
static void create_subthread();
...
int scanmanager(const struct optstruct *opts)
{
    ...
    menucreate(); // init lcd, menu, keypad
...
if( !optget(opts,"nocheckdb")->enabled ) {
	...
	mprintf("Check DB!\n");
        lprintf(1,0,0,"VT Denetleniyor");
        lprintf(0,0,1,"Lutfen Bekleyin");
        ...
        mprintf("Check DB End!\n");
        lprintf(1,0,0,"VT Denetleme    Tamamlandi!");
           ...
           // printf("! Dir : %s", file );
            ...
            if(LSTAT(file, &sb) == -1) {
                ...
                if(S_ISLNK(sb.st_mode)) {
                    ...
                    _scandirs(file, engine, opts, options, 1, sb.st_dev);
...
static void _scandirs(const char *dirname, struct cl_engine *engine, const struct optstruct *opts, unsigned int options, unsigned int depth, dev_t dev)
{
   ...
   // scandirs(dirname,engine,opts,options,depth,dev);
    // lprintf(1,0,0,"");
	...
	while(usbcheck==0)
		{
                    ...
                    usbway = chadd("sudo umount -f /dev/",usb);
                    ...
                    usbway2 = chadd("sudo mount /dev/",usb);
                    
                    usbway2 = chadd(usbway2, " /tmp/usbforscan/");
                    ...
                    printf("? : %s\n",usbway2);
                        ...
                        printf("Hızlı Tarama Yapılıyor..\n");
                        lprintf(1,0,0,"Bulundu!        Hizli Taraniyor..");
                        ...
                        scandirs("/tmp/usbforscan",engine,opts,options,depth,dev);
                         
                        _summary(opts);
                            //TODO here quick scan functions......
                        
                        //printf("Contuinue Full Scan?(Yes Press Green Button)");
                         lprintf(1,0,0,"Tam Tarama YaparDevam?  X - OK");
                        ...
                        {                        
                            //sleep(1);
                            lprintf(1,0,0,"Tam Tarama      Yapiliyor...");
                            recursion =  1;
                            scandirs("/tmp/usbforscan",engine,opts,options,depth,dev);
